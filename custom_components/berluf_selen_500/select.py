...
from .berluf_selen_500.funcs import Fan, Heater_cooler
...
async def async_setup_entry(
    ...
    async_add_entities(
        [
            ...
            Berluf_selen_500_fan_mode(
                entry=entry,
                entity_description=SelectEntityDescription(
                    key="berluf_selen_500",
                    name="Berluf Selen 500 fan mode",
                    options=Fan.Mode._member_names_,
                    # icon="mdi:format-quote-close",
                ),
            ),
        ]
...
class Berluf_selen_500_fan_mode(Berluf_selen_500_Entry, SelectEntity):
    """berluf_selen_500 heater cooler switch."""
    ...
    def __init__(
        ...
        """Initialize the sensor class."""
        super().__init__(entry, "fan_mode")
        self.entity_description = entity_description

        self._impl = Fan(entry.runtime_data.get_device())
    ...
    def select_option(self, option: str) -> None:
        """Change the selected option."""
        self._impl.set(Fan.Mode[option])
